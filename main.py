...
df["power_solar"] = df["pv_pr"].apply(lambda x: x * p_solar / 1000)  # [kW]

# Sum the power generated by wind turbines and solar panels
total_power_wt = df["power_wt"].sum()
total_power_solar = df["power_solar"].sum()
total_power = total_power_wt + total_power_solar
# Calculate the percentage contribution from each source
percentage_wt = (total_power_wt / total_power) * 100
percentage_solar = (total_power_solar / total_power) * 100
# Print the results
print(f"Total Power Generated: {total_power:.2f} units")
print(f"Wind Power: {total_power_wt:.2f} units ({percentage_wt:.2f}%)")
print(f"Solar Power: {total_power_solar:.2f} units ({percentage_solar:.2f}%)")
...
def process_combination(params):
    n_wind, p_solar, df_template, tank_capacity, constants = params
    (
        c_wind,
        c_solar,
        q_ro,
    ...
    # Unpack constants
    # c_wind, c_solar, etc., are unpacked from the constants tuple
    # Create a copy of the template DataFrame
    df = df_template.copy()
    # Calculate power generated by wind turbines
    df["power_wt"] = df["v_wind"].apply(wind_power_spline) * n_wind
    # Calculate power generated by solar panels
    df["power_solar"] = df["pv_pr"].apply(lambda x: x * p_solar / 1000)  # [kW]
    df["e_ro_hourly"] = e_ro_hourly
    # Simulate tank operations
    df_results = simulate_tank_operations(
        df=df,
        tank_capacity=tank_capacity,
        q_water=q_water,
    ...
    if df_results["tank_level"].min() >= 1000:
        tank_cost = calculate_tank_cost(tank_capacity)
        total_capex, _, _, _, _ = calculate_capex(
            c_wind=c_wind,
            n_wind=n_wind,
            c_solar=c_solar,
            p_solar=p_solar,
...
def find_viable_solutions_parallel(
    n_wind_vec, p_solar_vec, n_ro, tank_capacity, max_workers=12
):
    """
    Calculate viable CAPEX solutions in parallel for different n_wind and p_solar scenarios.
    Parameters:
    - n_wind_vec (list[int]): List of wind turbine counts to evaluate.
    - p_solar_vec (list[int]): List of solar power capacities (kWp) to evaluate.
    - n_ro (int): Number of desalination plants.
    - tank_capacity (float): Capacity of the tank in cubic meters.
    - max_workers (int): Number of parallel processes to use.
    Returns:
    - pd.DataFrame: Sorted DataFrame of viable solutions with columns ['n_wind', 'p_solar', 'total_capex'].
    """
    # Constants and parameters
    c_wind = 1340  # [euro/kW]
    c_solar = 800  # [euro/kWp]
    q_ro = 19200  # [m3/day]
    c_ro = 900  # [euro/m3*day]
    opex = 0.03  # [-]
    N = 20  # [years]
    i = 0.06  # [-]
    R = calculate_annuity_factor(i, N)
    q_water = 11.7e6  # [m3/year]
    wtr_cons_mnth = [6, 7, 7, 8, 9, 11, 12, 11, 9, 7, 7, 6]  # [%]
    sec_ro = 3.0  # [kWh/m3]
    q_ro_hourly = q_ro / 24
    e_ro_hourly = q_ro_hourly * sec_ro
    # Prepare the DataFrame template
    df_template = pd.DataFrame({"pv_pr": pv_pr, "v_wind": v_wind})
    # Generate all combinations of n_wind and p_solar
    combinations = list(itertools.product(n_wind_vec, p_solar_vec))
    # Prepare constants tuple to pass to worker
    constants = (
        c_wind,
        c_solar,
        q_ro,
    ...
    # Prepare parameters for each combination
    params_list = [
        (n_wind, p_solar, df_template, tank_capacity, constants)
        for n_wind, p_solar in combinations
    ]
    results = []
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_params = {
            executor.submit(process_combination, params): params
    ...
    if results:
        viable_solutions_df = pd.DataFrame(results)
        # Sort the DataFrame by total_capex
        viable_solutions_df = viable_solutions_df.sort_values(
            by="total_capex"
        ).reset_index(drop=True)
    else:
        viable_solutions_df = pd.DataFrame(columns=["n_wind", "p_solar", "total_capex"])
    return viable_solutions_df
...
viable_solutions_df = find_viable_solutions_parallel(
    n_wind_vec=n_wind_vec,
    p_solar_vec=p_solar_vec,
    n_ro=n_ro,
    tank_capacity=tank_capacity,
    max_workers=12,  # Utilize 12 CPU threads
)
